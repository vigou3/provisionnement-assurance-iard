...
$C_{i,j+1} = \lambda_j C_{i,j}$. Les coefficients $\lambda_k$ sont
estimés à l'aide des observations:
\begin{align*}
  \hat{\lambda}_k
  &= \frac{\sum_{i=1}^{n-k} C_{i,k+1}}{\sum_{i=1}^{n-k} C_{i,k}}, \quad
    k = 1, \dots, n-1.
\end{align*}
À partir de ces $\hat{\lambda}_k$, on peut estimer
\begin{align*}
  \hat{C}_{i,j}
  &= \left(
    \hat{\lambda}_{n+1-i} \times \hat{\lambda}_{n+2-i} \times \ldots
    \hat{\lambda}_{j-1}
    \right) C_{i,n+1-i}.
\end{align*}
